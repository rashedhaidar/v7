import React, { useState, useRef, useContext, useEffect } from 'react';
    ...
    export function Evaluation({ activities }: EvaluationProps) {
      ...
      const handleExport = () => {
        const exportData = activities.map(activity => {
          ...
          DAYS.forEach((_, dayIndex) => {
            ...
            const decisions = localStorage.getItem(`decisions-${weekKey}-${dayIndex}`)
            if (positiveNotes) {
              notes[`positiveNotes-${dayIndex}`] = JSON.parse(positiveNotes);
            }
            if (freeWriting) {
              notes[`freeWriting-${weekKey}-${dayIndex}`] = freeWriting;
            }
            if (decisions) {
              notes[`decisions-${weekKey}-${dayIndex}`] = decisions;
            }
          });
      ...
      const handleAddAchievement = () => {
        ...
        const savedAchievements = localStorage.getItem(`achievements-${weekKey}`);
        const currentAchievements = savedAchievements ? JSON.parse(savedAchievements) : [];
        const updatedAchievements = [...currentAchievements, ''];
        setAchievements(updatedAchievements);
        localStorage.setItem(`achievements-${weekKey}`, JSON.stringify(updatedAchievements));
      };
      ...
      return (
        <div className="p-6 bg-gradient-to-br from-purple-900/90 to-pink-900/90 rounded-lg shadow-lg" dir="rtl">
            ...
            <div className="bg-gradient-to-br from-violet-500/20 to-purple-500/20 p-6 rounded-lg">
              ...
              <div className="mt-4 flex flex-wrap justify-center gap-4">
                {LIFE_DOMAINS.map(domain => {
                  ...
                  return (
                    <div key={domain.id} className="bg-black/20 p-4 rounded-lg flex flex-col items-center w-fit">
                      ...
                      <p className={`text-sm ${domainColors[domain.id] || 'text-white'}`}>
                ...
                {DAYS.map((_, dayIndex) => {
                  ...
                  return freeWriting && (
                    <div key={dayIndex} className="mb-2">
                      <h4 className="text-white/70 text-sm">{DAYS[dayIndex]} - {formatDate(currentDate)}</h4>
